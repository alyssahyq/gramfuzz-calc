...
class Fuzz_runner(object):
    def __init__(self, benchmark='bc', cycle_num=1, file_num=100, timeout=1):
    ...
    def run(self):
        ...
        for round in bar1:
            ...
            for i in bar2:
                ...
                cur_time = (datetime.datetime.now() - start_time).seconds
                timing = 'Runtime:  {0} hrs, {1} min, {2} sec'.format(cur_time // 3600, (cur_time - cur_time // 3600) // 60, cur_time % 60) + '\t\t\t\t\t\t\t\t\t\t'
                overall = 'Unique Hangs:  ' + str(self.hangs) + '\t' + 'Unique Crashes:  ' + str(self.exceptions) + '\t' + 'Interprete Errors:  ' + str(self.errors)
