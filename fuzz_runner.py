...
import subprocess
import sys
from generator import Generator, GramGenerator, MutateGenerator
from tqdm.auto import trange


class FuzzRunner(object):
    def __init__(self, 
                gen_strategy: type(Generator), 
                feed: str, 
                benchmark: str = 'bc', 
                cycle_num: int = 1, 
                file_num: int = 100, 
        ...
        self._exceptions = 0
        self._start_time = 0
        self._gen = gen_strategy(feed, file_num)
        if benchmark == 'bc':
    ...
    def run(self):
        ...
        for round in bar1:
            ...
            bar1.set_description('Total Progress: ')
            self._gen.generate()
            bar2 = trange(self._file_num)
            for i in bar2:
                self._cur_file = i + 1
                bar2.set_description('Round {0} Progress: '.format(self._cur_cycle))
                ...
                timing = 'Runtime:  {0} hrs, {1} min, {2} sec'.format(cur_time // 3600, (cur_time - (cur_time // 3600) * 3600) // 60, cur_time % 60) + '\t\t\t\t\t\t\t\t\t\t'
...
if __name__ == "__main__":
    runner = FuzzRunner(GramGenerator, 'gram_seed/arith_grammar.py')
    runner.run()
